 ...
 *   --skip-tests          Skip bun test (still runs bun install + bun check)
...
function stepVerify(opts: CliOptions): void {
	...
	if (opts.dryRun) {
		...
		if (!opts.skipTests) console.log("[DRY-RUN] Would run: bun test");
		return;
	}
	...
	if (opts.skipTests) {
		setStatus("bun test", "SKIPPED");
		console.log("[SKIPPED] bun test (--skip-tests)");
	} else {
		runCheckedStep("bun test", ["bun", "test"]);
	}
}
