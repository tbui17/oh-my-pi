...
interface EvalAgentArgs {
  ...
  * (default `false`). Pass `true` for long-lived conversational subagents
  * that the caller will message via IRC after the initial run completes.
  * When `true`, the subagent goes `idle` instead of being disposed, armed
  * with the standard idle-TTL park timer.
  */
 keepAlive?: boolean;
}
