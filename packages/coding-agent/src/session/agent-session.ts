...
interface CompactionModelCandidate extends Model {
...
export class AgentSession {
	...
	async #maintainAdvisorContext(advisor: ActiveAdvisor, incomingTokens: number): Promise<boolean> {
		...
		for (const model of candidates) {
			const apiKey = await this.#modelRegistry.getApiKey(model, advisorProviderSessionId);
			if (!apiKey) continue;
			...
			try {
				compactResult = await compact(
					preparation,
					model,
					this.#modelRegistry.resolver(model, advisorProviderSessionId),
					undefined,
					undefined,
					{
	...
	#resolveCompactionModelCandidates(
		...
		const addCandidate = (model: Model | undefined, thinkingLevel?: ThinkingLevel): void => {
			...
			candidates.push(Object.assign(model, { thinkingLevel }) satisfies CompactionModelCandidate);
		};
		...
		// Prefer the caller's preferred model (typically the active session's
		// model): it's what the user is actively using, and routing compaction
		// to a different provider (e.g. an OpenAI default model while the chat
		// is on Anthropic) changes provider-specific behavior like remote
		// compaction endpoints. Role-based candidates only kick in as auth
		// fallbacks when the current model has no usable credentials.
		if (preferredModel) {
	...
	async #compactWithFallbackModel(
		...
		for (const model of candidates) {
			...
			try {
				...
				if (!AIError.is(AIError.classify(error, model.api), AIError.Flag.AuthFailed)) {
					throw error;
				}
			}
		}
				...
				for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex++) {
					...
					while (true) {
						try {
							...
							const message = error instanceof Error ? error.message : String(error);
							const id = AIError.classify(error, model.api);
