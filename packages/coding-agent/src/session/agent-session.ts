...
export class AgentSession {
	...
	/**
	 * Upstream compatibility hook used by legacy extensions that construct sessions
	 * directly and expect the session to initialize extension runtime actions.
	 */
	async bindExtensions(options: { uiContext?: ExtensionUIContext } = {}): Promise<void> {
		const runner = this.#extensionRunner;
		if (!runner) return;
		...
		runner.initialize(
			{
				sendMessage: (message, sendOptions) => {
					this.sendCustomMessage(message, sendOptions).catch(error => {
						logger.warn("Extension sendMessage failed", {
							error: error instanceof Error ? error.message : String(error),
						});
					});
				},
				sendUserMessage: (content, sendOptions) => {
					this.sendUserMessage(content, sendOptions).catch(error => {
						logger.warn("Extension sendUserMessage failed", {
							error: error instanceof Error ? error.message : String(error),
						});
					});
				},
				...
				setSessionName: async name => {
					await this.sessionManager.setSessionName(name, "user");
				},
			},
			{
				getModel: () => this.model,
				...
				compact: async instructionsOrOptions => {
					...
					const compactOptions =
						instructionsOrOptions && typeof instructionsOrOptions === "object" ? instructionsOrOptions : undefined;
					await this.compact(instructions, compactOptions);
				},
			},
			{
				getContextUsage: () => this.getContextUsage(),
				...
				newSession: async newOptions => {
					const success = await this.newSession({ parentSession: newOptions?.parentSession });
					if (success && newOptions?.setup) {
						await newOptions.setup(this.sessionManager);
					}
					return { cancelled: !success };
				},
				branch: async entryId => {
				...
				navigateTree: async (targetId, navOptions) => {
					const result = await this.navigateTree(targetId, { summarize: navOptions?.summarize });
					return { cancelled: result.cancelled };
				},
				switchSession: async sessionPath => {
					const success = await this.switchSession(sessionPath);
